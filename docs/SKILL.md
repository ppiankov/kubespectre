...
Scans Kubernetes clusters for security findings.
...
- Does not remediate or modify Kubernetes clusters — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated Kubernetes clusters monitoring — point-in-time security audit only
...
```bash
kubespectre scan --output json | jq '.summary'
kubespectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
