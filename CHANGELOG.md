...
### Fixed
- The sample RBAC ClusterRole generated by `kubespectre init` now grants `get`/`list` on the cluster-scoped `persistentvolumes` resource; without it, the new PersistentVolume auditor above would fail with an RBAC denial for anyone following the documented init workflow
