...
### Changed
- `STALE_SECRET` findings on secrets carrying a recognized controller-managed annotation marker (defaulting to `cert-manager.io/` and `external-secrets.io/`) are now reported at `medium` severity instead of `high`, configurable via `managed_secret_markers` and `disable_managed_secret_downranking`
